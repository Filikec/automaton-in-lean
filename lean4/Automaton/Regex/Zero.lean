...
def zero_nfa : NFA σs := {q := Nat, qs := {}, q₀ := {}, fs := {}, δ := fun _ _ => {}}

theorem accepts_iff (w : word σs) : nfa_accepts zero_nfa w ↔ False := by
  apply Iff.intro
  · intro na
    simp only [zero_nfa,nfa_accepts,Finset.Nonempty] at na
