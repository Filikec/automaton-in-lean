...
theorem word_cycle {w : word σs} {s t : qs} (hlen : qs.card ≤ w.length)
    (hx : δ_star' dfa s w = t) :
    ∃ q a b c,
      w = a ++ b ++ c ∧
        a.length + b.length ≤ qs.card ∧
          b ≠ [] ∧ δ_star' dfa s a = q ∧ δ_star' dfa q b = q ∧ δ_star' dfa q c = t := by
  have ⟨n, m, hneq, heq⟩  :=
    Fintype.exists_ne_map_eq_of_card_lt
      (fun n : Fin (Fintype.card qs + 1) => δ_star' dfa s (w.take n)) (by simp)
  wlog hle : (n : ℕ) ≤ m
  ...
  · intro h
    ...
    · apply hneq
      ...
      assumption'
    exact hm.trans hlen
  have hq : δ_star' dfa (δ_star' dfa s ((w.take m).take n)) ((w.take m).drop n) =
      δ_star' dfa s ((w.take m).take n) := by
    rw [List.take_take, min_eq_left hle, ←δ_star'_append_eq, heq, ← min_eq_left hle, ←
      List.take_take, min_eq_left hle, List.take_append_drop]
  use hq
  rwa [← hq, ← δ_star'_append_eq, ← δ_star'_append_eq, ← List.append_assoc,
...
theorem pumping_lemma {w : word σs} (hlen : qs.card ≤ w.length) (hw : w ∈ dfaLang dfa) :
    ∃ a b c,
      w = a ++ b ++ c ∧
        a.length + b.length ≤ qs.card ∧
          b ≠ [] ∧ ∀ n, (a ++ listPower b n ++ c) ∈ dfaLang dfa := by
